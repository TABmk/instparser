...
instParser.prototype.getUser = function () {
  return new Promise((resolve, reject) => {
    request(`http://instagram.com/${this.name}`).then(res => res.text()).then(body => {
      ...
      let data = JSON.parse(body.match(/<script type="text\/javascript">(.*)<\/script>/g)[0].slice(52,-10));
      data.entry_data.ProfilePage ? resolve(data.entry_data.ProfilePage[0].graphql.user) : reject('User not found');
    }).catch(e => reject(e));
  });
};
